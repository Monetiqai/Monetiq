-- ============================================================================
-- MINIMAL MIGRATION - CORE FUNCTIONALITY ONLY
-- This version works without superuser permissions
-- ============================================================================
...
SELECT 'RLS enabled on generations' as check, 
       (SELECT relrowsecurity FROM pg_class WHERE relname = 'generations')::text as result
UNION ALL
SELECT 'inputs bucket created', 
       EXISTS(SELECT 1 FROM storage.buckets WHERE id = 'inputs')::text
UNION ALL
SELECT 'user_quotas table created', 
       EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_quotas')::text;

-- ============================================================================
-- DONE! The webhook will be configured manually via Edge Function
-- ============================================================================
