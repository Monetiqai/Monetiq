/**
 * PHASE 11 — END-TO-END TEST WITH DB PERSISTENCE
 * 
 * Tests complete pipeline with database persistence
 * 
 * ACCEPTANCE CRITERIA:
 * - Record created at start (status='generating')
 * - 4 shot URLs updated in DB
 * - Status finalized (status='success')
 * - GET endpoint returns the record
 */
...
import { getGenerationByRunId } from './lib/ads-mode/db-helpers';
import * as fs from 'fs';

// Test product (matching Phase 10)
const TEST_PRODUCT = {
    productId: 'test-hoodie-phase11',
    productName: 'Plain Hoodie (Phase 11)',
...
async function runPhase11Test() {
    console.log(`\n╔════════════════════════════════════════╗`);
    console.log(`║  PHASE 11 — E2E TEST (WITH DB)         ║`);
    console.log(`╚════════════════════════════════════════╝\n`);
    ...
    try {
        ...
        // Verify DB record
        console.log(`\n[DB] Verifying database record...`);
        const dbRecord = await getGenerationByRunId(result.runId);

        if (!dbRecord) {
            throw new Error('DB record not found!');
        }

        console.log(`[DB] ✓ Record found in database`);
        console.log(`[DB]   ID: ${dbRecord.id}`);
        console.log(`[DB]   Status: ${dbRecord.status}`);
        console.log(`[DB]   Shot 1 URL: ${dbRecord.shot_1_url ? '✓' : '✗'}`);
        console.log(`[DB]   Shot 2 URL: ${dbRecord.shot_2_url ? '✓' : '✗'}`);
        console.log(`[DB]   Shot 3 URL: ${dbRecord.shot_3_url ? '✓' : '✗'}`);
        console.log(`[DB]   Shot 4 URL: ${dbRecord.shot_4_url ? '✓' : '✗'}`);
        ...
        const validations = {
            'Generation ID present': !!result.generationId,
            ...
            'DB record exists': !!dbRecord,
            'DB status is success': dbRecord?.status === 'success',
            'DB has all 4 URLs': !!(dbRecord?.shot_1_url && dbRecord?.shot_2_url && dbRecord?.shot_3_url && dbRecord?.shot_4_url),
        ...
        const logDir = path.join(process.cwd(), 'test-logs', 'phase11');
        ...
        if (allPassed && result.status === 'SUCCESS') {
            console.log(`║  ✅ PHASE 11 ACCEPTANCE: PASS          ║`);
        } else {
            console.log(`║  ❌ PHASE 11 ACCEPTANCE: FAIL          ║`);
        }
        console.log(`╚════════════════════════════════════════╝\n`);
